    ...
    <div class="relative max-h-screen overflow-hidden">
        @if ($collection->is_youtube_link)
            <div class="videoWrapper relative h-0" style="padding-bottom: 56.25%;">
                <iframe class="absolute top-0 left-0 w-full h-full" src="https://www.youtube.com/embed/{{ $collection->youtube_link}}?autoplay=1&mute=1" frameborder="0" allowfullscreen></iframe>
            </div>
        @else
            <video
